...
def get_fanju(id,url):
    ...
    aid_json=requests.get(url,headers=header)
    s=aid_json.text
    contents=json.loads(s)
...
python3 -f epid  番剧视频爬取(不含大会员)"""
    ...
    try:
        ...
        try:
            video_api = "https://api.bilibili.com/x/player/pagelist?bvid=%s&jsonp=jsonp" % id[0]
            get_video(id=id, url=video_api)
        except:
